...
def setup_results_table(conn):
    """Creates the 'incident_reports' table if it doesn't exist."""
...
# MODIFIED: This function now reads from the 'actionable_tweets' table
def fetch_actionable_tweets(conn):
    """Fetches actionable tweets that have not yet been processed into incident_reports."""
    print("Fetching unprocessed actionable tweets from the database...")
    with conn.cursor() as cursor:
        # MODIFIED: This query targets 'actionable_tweets' as the source
        query = """
            SELECT
                at.source_tweet_id as id,
                at.original_tweet_text as text
            FROM actionable_tweets at
            LEFT JOIN incident_reports ir ON at.source_tweet_id = ir.original_tweet_id
            WHERE ir.original_tweet_id IS NULL;
        """
        cursor.execute(query)
        colnames = [desc[0] for desc in cursor.description]
        records = [dict(zip(colnames, row)) for row in cursor.fetchall()]
        print(f"✅ Found {len(records)} new actionable tweets to process.")
        return records
...
if __name__ == "__main__":
    ...
    if conn:
        try:
            ...
            # MODIFIED: Call the updated function
            tweets_to_process = fetch_actionable_tweets(conn)

            if not tweets_to_process:
                print("No new actionable tweets to process. The database is up-to-date.")
            else:
                print(f"\n--- Starting to process {len(tweets_to_process)} actionable tweets ---")
                for i, tweet in enumerate(tweets_to_process):
                    print(f"Processing actionable tweet {i + 1}/{len(tweets_to_process)} (ID: {tweet['id']})...")
                    ...
                    if extracted_data:
                        ...
                        print("   -> Successfully processed and saved to incident_reports.")
                    else:
