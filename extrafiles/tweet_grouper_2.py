...
def setup_database_schema(conn):
    """Ensures the final_report table exists."""
    with conn.cursor() as cur:
        # MODIFIED: Removed the ALTER TABLE command for the status column.
        cur.execute("""
...
def fetch_unprocessed_incidents(conn):
    """Fetches incident reports that are not yet present in any final_report."""
    with conn.cursor() as cur:
        # MODIFIED: This query now checks against the final_report table directly.
        cur.execute("""
            SELECT 
                ir.id, 
                ir.original_tweet_text, 
                ir.extracted_location, 
                ir.extracted_issue 
            FROM incident_reports ir
            WHERE NOT EXISTS (
                SELECT 1 
                FROM final_report fr 
                WHERE ir.id = ANY(fr.source_incident_ids)
            );
        """)
    ...
    if db_connection:
        try:
            ...
            if not incidents:
                ...
                texts_for_embedding = [inc['original_tweet_text'] for inc in incidents]
                ...
                if embeddings is not None:
                    ...
                    db = DBSCAN(eps=0.35, min_samples=2, metric='cosine').fit(embeddings)
                    labels = db.labels_
                    ...
                    for label in unique_labels:
                        ...
                        # MODIFIED: The status update function call has been removed.
                        print(f"   -> ✅ Saved summary to 'final_report' for {len(incident_ids_in_cluster)} source incidents.")
                        time.sleep(1)
