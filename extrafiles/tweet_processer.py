...
def fetch_unprocessed_tweets(conn):
    ...
    with conn.cursor() as cursor:
        # This query selects only the tweets that haven't been processed yet.
        query = """
...
if __name__ == "__main__":
    ...
    if conn:
        try:
            # 1. Ensure the new results table exists
            setup_results_table(conn)

            # 2. Fetch only new tweets that haven't been processed
            ...
            if not tweets_to_process:
                ...
                for i, tweet in enumerate(tweets_to_process):
                    ...
                    # 3. Extract details using Gemini
                    extracted_data = extract_incident_details(tweet['text'])
                    
                    if extracted_data:
                        # 4. Prepare data and insert into the new table
                        report = {
